            ...
            <div class="flex mx-auto mt-10 ">
                <form method="POST" action="{{ route('ticket_gate-page', ['id' => $live_stream->id]) }}" class="w-[100%] mx-auto bg-red-900 px-10 py-10 border rounded-lg">
                    @csrf
                        ...
                        @if($tracking_info->email === 'sim')
                            <div class="mb-4">
                                <label for="email" class="text-lg font-bold text-black mb-2">Email:</label>
                                <input type="text" name="email" value="" class="w-full mt-2 p-2 rounded-lg">
                            </div>
                        @endif

                        @if($tracking_info->name === 'sim')
                            <div class="mb-4">
                                <label for="name" class="text-lg font-bold text-black mb-2">Name:</label>
                                <input type="text" name="name" value="" class="w-full mt-2 p-2 rounded-lg">
                            </div>
                        @endif

                        @if($tracking_info->phone === 'sim')
                            <div class="mb-4">
                                <label for="phone" class="text-lg font-bold text-black mb-2">Phone:</label>
                                <input type="text" name="phone" value="" class="w-full mt-2 p-2 rounded-lg">
                            </div>
                        @endif
